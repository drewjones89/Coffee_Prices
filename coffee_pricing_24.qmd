---
title: "Pricing Among the Best of the Best"
author: "Drew Jones"
subtitle: Price analysis of top US coffee roasters in 2024
...
```{r}
#| label: data_dictionary_show
#| tbl-cap: "Figure 1: Data Dictionary"
#| echo: false
...
#| label: eda_character
#| echo: false
#| tbl-cap: "Figure 2: EDA for Character Variables"
skim(coffee_clean) |>
...
#| label: eda_numeric
#| echo: false
#| tbl-cap: "Figure 3:EDA for Numeric Variables"
skim(coffee_clean) |>
...
*NOTE: Is it possible to get this on two rows maybe?*
...
coffee_clean |>
  ...
  labs(
    ...
    scale_fill_viridis_c(option = "magma", begin = 0.2, end = 0.8, name = "% Missing") +
    theme_few() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

```{r}
#| label: price_histogram
#| echo: false
#| warning: false
...
*NOTE: Change the color to just be magma*
...
```{r}
#| label: table_small_bags
#| echo: false
#| tbl-cap: "Figure 6: Blending Accoutns for Most of the Small Bags"
coffee_clean |>
  filter(bag_size < 7.5) |>
  arrange(desc(adj_price)) |>
  mutate(across(where(is.numeric),  \(x) round(x, 2))) |>
  select(bag_size, name, roaster, adj_price,
        origin_state, process, varietal1, varietal2) |>
  flextable() |>
  theme_vanilla()
```

*Figure 6* shows that most of these 7 small bags come from a single roaster, Blendin. 

```{r}
#| label: tbl_over_100
#| echo: false
#| tbl-cap: "Coffees over $100 for 12 oz bag"
coffee_clean |>
...
coffee_clean |>
  ...
  labs(x = "Bag size (oz)",
       ...
       caption = "Figure 6",
       title = "Small bags are are both rare and expensive",
...
```{r}
#| label: tbl_roaster_price_averages
#| echo: false
#| tbl-cap: "Prodigal stands out as the most expensive"
coffee_clean |>
  filter(roaster != "Botz") |>
  group_by(roaster) |>
  summarize(
